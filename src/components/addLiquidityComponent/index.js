...
const LiquidityModal = ({ isVisible, handleClose, closeBtn }) => {
  ...
  const _handleSupply = async () => {
    try {
      ...
      dispatch(GetPoolPositionAction(web3context, contractUniswapPair));
      console.log("Dispatched GetPoolPositionAction");
  ...
  const OnChangeHandler = async (val, tokenName, isMaxButton) => {
    ...
    if (tokenName === "phnx") {
      ...
      total = total + parseFloat(reserve1.toFixed(2));
      ...
      <div className="addLiquidityBox">
        <div style={{ marginBottom: "9px" }}>
          <div style={styles.divTopHeading}>
            ...
            >
              Add liquidity to the ETH/NODT pool <br /> and receive LP tokens
              <img
        ...
        {web3context.active == false ? (
          ...
          <Button
            ...
            style={{
              ...styles.btnAddLiquidity,
              backgroundColor:
                loading || approveStatus != 0 ? "#acacac" : "#332545",
          ...
          >
            {approveStatus == 0 ? "Approve NODT" : "Approving NODT..."}
          </Button>
        )}
      </div>
...
const styles = {
  ...
  containerTip: {
    ...
    padding: "10px 15px",
    background:
      "linear-gradient(90deg, rgba(51, 37, 69, 0.55) 0%, rgba(233, 213, 255, 0.55) 143.12%)",
    borderRadius: 15,
    // marginBottom: 20,
  },
  txtTipParagraph: {
