...
import SlippingTolerancexxx from "../connectModal/SlippingTolerancexxx";
import SettingsLogo from "../../assets/settings.png";
import { fixedWithoutRounding } from "../../utils/formatters";
import BigNumber from "bignumber.js";

const SwapModal = ({ isVisible, handleClose, closeBtn }) => {
  ...
  const [slippageModalxxx, setSlippageModalxxx] = useState(false);
  ...
  const swapModelNewTest = useSelector(
    (state) => state.localReducer.slippageAddLiquidity
  );
  ...
  const ETH_STATION = "https://app.defisaver.com/api/gas-price/current";
  ...
  const getCurrentGasPrices = async () => {
    try {
      ...
      let prices = {
        low: response.cheap / 10,
        medium: response.regular / 10,
  ...
  const handleSwap = () => {
    // Ensure phnxValue and ethValue are not undefined or null
    if (phnxValue !== undefined && ethValue !== undefined) {
      // Swap values
      setPhnxValue(ethValue);
      setEthValue(phnxValue);
    }
  };
  ...
  return (
    <Box sx={styles.containerStyle} className="modal-scroll">
      ...
      <div className="dialog-style" style={{ marginTop: "25px" }}>
        ...
        <div style={{ position: "relative", marginTop: "25px" }}>
          <div className="token-container">
            <div className="token-wrapper" onClick={() => setSlippageModalxxx(!slippageModalxxx)}>
              <img alt="logo" style={styles.imgLogoPhnx} src={PhnxLogo} />
              <div style={styles.containerImg}>
                <Typography style={styles.txtPhnx}>NODT</Typography>
              </div>
              <img alt="dropdown" style={{ marginLeft: "auto" }} src={DropdownLogo} />
            </div>
            <div style={styles.containerInput}>
              <div style={styles.divPhnxAmount}>
                <Typography style={styles.txtInput}>Available NODT:</Typography>
                <Typography style={styles.txtAmount}>
                  {balancePhnx ? `${balancePhnx}` : "0.0"} NODT
                </Typography>
              </div>
                    ...
                    endAdornment: (
                      <IconButton
                        ...
                        onClick={() => {
                          ...
                          if (balanceEth > 0) {
                            OnChangeHandler(balanceEth, "eth", true);
                          }
        ...
        {/* <div className="container-pool-share">
          <div style={styles.txtDivPhEth}>
            <Typography style={styles.txtConvDetails}>
              {phnxPerEth ? phnxPerEth : "0.0"} NODT / ETH
            </Typography>
            <Typography style={styles.txtConvDetails}>
              {ethPerPhnx ? ethPerPhnx : "0.0"} ETH / NODT
            </Typography>
          </div>
        ...
        {web3context.active == false ? (
          ...
          <Button
            ...
            style={{
              ...styles.btnAddLiquidity,
              backgroundColor:
                loading || approveStatus != 0 ? "#acacac" : "rgb(51, 37, 69)",
          ...
          >
            {approveStatus == 0 ? "Submit" : "Processing"}
          </Button>
        )}
      </div>
      ...
      <SlippingTolerancexxx
        status={slippageModalxxx}
        handleClose={setSlippageModalxxx}
        slippageValue={slippageAddLiquidityxxx}
        slippageType="add"
      />
